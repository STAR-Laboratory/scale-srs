...
print(STDERR "\t--i <input/suite>      : input suite from bench_common_rrs1K \n");
...
$mysim = "$dest_dir"."/"."sim_rrs1K.bin";
#$myopt = "$dest_dir"."/"."sim_rrs1K.opt";
...
die "No benchmark set '$wsuite' defined in bench_common_rrs1K.pl\n"
    unless $SUITES{$wsuite};
@w = split(/\n/, $SUITES{$wsuite});
$num_w = scalar @w;


die "No benchmark set '$isuite' defined in bench_common_rrs1K.pl\n"
